...
public class ModelMapperConfig {
    ...
    private void customerToOrderedCustomersDto(ModelMapper mapper) {
        Converter<LocalDate, String> toDateToString =
                ctx -> DateTimeFormatter.ISO_LOCAL_DATE_TIME.format(
                        LocalDateTime.of(ctx.getSource(), LocalTime.MIN));

        mapper.createTypeMap(Customer.class, OrderedCustomersDto.class)
                .addMappings(mpr -> mpr.using(toDateToString).map(Customer::getBirthDate,
                        OrderedCustomersDto::setBirthDate));
    }
}
